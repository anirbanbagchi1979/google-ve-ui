...
const OperationsPanel = ({ operations, addLog, onVideoSelect, onStatusUpdate }: OperationsPanelProps) => {
  ...
  const handleStatusCheck = async (op: Operation) => {
    ...
    const endpoint = `https://${config.location}-aiplatform.googleapis.com/v1/projects/${config.projectId}/locations/${config.location}/publishers/google/models/${op.type === 'upscale' ? config.upscaleModel : config.videoGenModel}:fetchPredictOperation`;
